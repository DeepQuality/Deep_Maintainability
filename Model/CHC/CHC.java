...
public class CHC extends Metodo {
    ...
    public void ejecutar() {
        ...
        for (int m = 0; m < datosTest.length; m++) {
            StringBuilder stringBuilder = new StringBuilder();
            for (int n = 0; n < datosTest[m].length; n++) {
                stringBuilder.append(datosTest[m][n]);
                stringBuilder.append(" ");
            }
            stringBuilder.append(clasesTest[m] + "\n");
            Files.addToFile("normalized_test.txt", stringBuilder.toString());
        }
        ...
        List<Cromosoma> cromosomas = Arrays.asList(poblacion);

        int finalNClases = nClases;
        cromosomas.parallelStream().forEach(new Consumer<Cromosoma>() {
            @Override
            public void accept(Cromosoma cromosoma) {
                cromosoma.evalua(datosTrain, realTrain, nominalTrain, nulosTrain, clasesTrain, alfa, kNeigh, finalNClases, distanceEu);
            }
        });
...
//		}
        ...
        while (ev < nEval) {
            ...
            List<Cromosoma> cromosomas1 = Arrays.asList(newPob);
            int finalNClasesTemp = nClases;
            cromosomas1.parallelStream().forEach(new Consumer<Cromosoma>() {
                @Override
                public void accept(Cromosoma cromosoma) {
                    cromosoma.evalua(datosTrain, realTrain, nominalTrain, nulosTrain, clasesTrain, alfa, kNeigh, finalNClasesTemp, distanceEu);
                }
            });
            ev += newPob.length;
...
//            }
            ...
            if (d < 0) {
                ...
                List<Cromosoma> cromosomas2 = new ArrayList<>();
                int finalNClasesTemp2 = nClases;
                for (i = 0; i < tamPoblacion; i++) {
                    if (!(poblacion[i].estaEvaluado())) {
                        ev++;
                        cromosomas2.add(poblacion[i]);
                    }
                }

                cromosomas2.parallelStream().forEach(new Consumer<Cromosoma>() {
                    @Override
                    public void accept(Cromosoma cromosoma) {
                        cromosoma.evalua(datosTrain, realTrain, nominalTrain, nulosTrain, clasesTrain, alfa, kNeigh, finalNClasesTemp2, distanceEu);
                    }
                });
...
//                    }
        ...
        String Subset = ficheroSalida[0].substring(11);
        ...
        for (int m = 0; m < conjS.length; m++) {
            StringBuilder stringBuilder = new StringBuilder();
            for (int n = 0; n < conjS[m].length; n++) {
                stringBuilder.append(conjS[m][n]);
                stringBuilder.append(" ");
            }
            stringBuilder.append(clasesS[m] + "\n");
            Files.addToFile("instances.txt", stringBuilder.toString());
        }
    ...
    public void leerConfiguracion(String ficheroScript) {
        ...
        tokens = new StringTokenizer(linea, "=");
        tokens.nextToken();
        distanceEu = tokens.nextToken().substring(1).equalsIgnoreCase("Euclidean");
